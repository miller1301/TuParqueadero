...
import { InfoReservasPage } from './info-reservas/info-reservas.page';
import { ModalController } from '@ionic/angular';
import { InforeservaPage } from './inforeserva/inforeserva.page';
...
export class ReservasPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.firestore.getAllDocs('ReservasL').subscribe( (res: any) =>{
      this.arreglo = [];
    ...
    this.firestore.getAllDocs('reservas').subscribe( (res: any) =>{
      this.arreglo3 = [];
      res.forEach( (parqueadero:any) => {
        this.arreglo.push({
          IdParqueadero: parqueadero.payload.doc.id,
      ...
      this.arreglos4 = this.arreglo.filter( (item)=>{ 
        if(1==1){
          return true 
        }
      })
  ...
  async presentModal2(index: number) {
    const modal = await this.modalController.create({
      component: InforeservaPage,
      cssClass: 'my-custom-class',
      componentProps: {
        Parqueadero: this.arreglos4[index]
      }
    });
    return await modal.present();
  }
